...
# CA2 = read.table("clipboard",header = TRUE)                                  # data2则来源于all_ep
# save(CA2,file = "D:/Rproject/pre_and_act/CA2.RData")
load("D:/Rproject/pre_and_act/CA2.RData")
...
results1 = calculate_metrics(CA2$area, CA2$CA21)
results2 = calculate_metrics(CA2$area, CA2$CA22)
results3 = calculate_metrics(CA2$area, CA2$CA23)
results4 = calculate_metrics(CA2$area, CA2$CA24)
results5 = calculate_metrics(CA2$area, CA2$CA25)

# 绘制实际值vs理论值的散点图，并添加一条y=x的45度线
p1 = ggplot(data = CA2, aes(x = area, y = CA21)) +
...
p1 = p1 + annotate("text", x = 6, y = 40, label = bquote(R^2 == .(round(results1$R_Squared, 2))), size = 6)
p1 = p1 + annotate("text", x = 6, y = 35, label = bquote(RMSE == .(round(results1$RMSE, 2))), size = 6)
p1 = p1 + annotate("text", x = 20, y = 0, label = "(c)", size = 8)
p1
