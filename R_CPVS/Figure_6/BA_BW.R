...
model = lm(area ~ width, data = data11)
#model = lm(log(area) ~ log(width), data = data11)
...
#equation = paste("ln(y) =", slope, "ln(x) +", abs(intercept))
r_squared = round(summary_model$r.squared, digits = 2)                          # R方

Scatter = ggplot(data11, aes(x = width, y = area))+
  ...
  labs(x = "Width", y = "Area")+
  annotate("text", x = min(data11$width), y = max(data11$area), 
...
ggsave("AW1.pdf", Scatter, width = 6.5, height = 5.5, unit = "in", dpi = 300)
